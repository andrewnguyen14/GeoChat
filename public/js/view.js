            ...
            if (data || $("#newPassword").val() !== $("#confirmPassword").val()) {
              $("#successmessage").html("<div class='alert alert-danger fade in'>" +
                ...
                $("#successmessage").html("<div class='alert alert-success fade in'>" +
                  "<a href='#' class='close' data-dismiss='alert'>&times;</a>" +
                  "<strong>'Success!'</strong> " + 'Your account has been made successfully.' + ' ' + '<button type="button" class="btn btn-info btn-lg navbar-btn signInNav" data-toggle="modal" data-target="#signInModal">Sign In!</button>' + 
                  "</div>");
                $("#newName").val("");
                $("#newUsername").val("");
                $("#newPassword").val("");
